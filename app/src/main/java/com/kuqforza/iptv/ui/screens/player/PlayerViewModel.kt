...
import com.kuqforza.iptv.cast.CastConnectionState
...
import com.kuqforza.iptv.di.MainPlayerEngine
import com.kuqforza.iptv.player.LivePreviewHandoffManager
import com.kuqforza.iptv.ui.model.orderedByRequestedRawIds
import com.kuqforza.iptv.util.isPlaybackComplete
...
import com.kuqforza.data.security.CredentialDecryptionException
...
import com.kuqforza.domain.usecase.MarkAsWatched
...
import com.kuqforza.player.Media3PlayerEngine
import com.kuqforza.player.PlaybackState
import com.kuqforza.player.PlayerEngine
...
class PlayerViewModel @Inject constructor(
    ...
    private val favoriteRepository: com.kuqforza.domain.repository.FavoriteRepository,
    internal val playbackHistoryRepository: PlaybackHistoryRepository,
    private val providerRepository: com.kuqforza.domain.repository.ProviderRepository,
    ...
    internal val currentChannelFlow = MutableStateFlow<com.kuqforza.domain.model.Channel?>(null)
    val currentChannel: StateFlow<com.kuqforza.domain.model.Channel?> = currentChannelFlow.asStateFlow()
    ...
    private val currentChannelFlowList = MutableStateFlow<List<com.kuqforza.domain.model.Channel>>(emptyList())
    val currentChannelList: StateFlow<List<com.kuqforza.domain.model.Channel>> = currentChannelFlowList.asStateFlow()

    internal val recentChannelsFlow = MutableStateFlow<List<com.kuqforza.domain.model.Channel>>(emptyList())
    val recentChannels: StateFlow<List<com.kuqforza.domain.model.Channel>> = recentChannelsFlow.asStateFlow()
     ...
     */
    internal var channelList: List<com.kuqforza.domain.model.Channel> = emptyList()
        set(value) {
            field = value
            rebuildChannelNumberIndex()
        }
    internal var channelNumberIndex: Map<Int, com.kuqforza.domain.model.Channel> = emptyMap()
        private set
    ...
    internal fun logRepositoryFailure(operation: String, result: com.kuqforza.domain.model.Result<Unit>) {
        if (result is com.kuqforza.domain.model.Result.Error) {
    ...
    val playerStats: StateFlow<com.kuqforza.player.PlayerStats> =
        activeEngineState(com.kuqforza.player.PlayerStats()) { it.playerStats }
    val availableAudioTracks: StateFlow<List<com.kuqforza.player.PlayerTrack>> by lazy(LazyThreadSafetyMode.NONE) {
        activeEngineState(emptyList()) { it.availableAudioTracks }
    }
    val availableSubtitleTracks: StateFlow<List<com.kuqforza.player.PlayerTrack>> by lazy(LazyThreadSafetyMode.NONE) {
        activeEngineState(emptyList()) { it.availableSubtitleTracks }
    }
    val availableVideoQualities: StateFlow<List<com.kuqforza.player.PlayerTrack>> by lazy(LazyThreadSafetyMode.NONE) {
    ...
    internal suspend fun preparePlayer(
        streamInfo: com.kuqforza.domain.model.StreamInfo,
        requestVersion: Long
    ): Boolean {
    ...
    private suspend fun probePlaybackUrl(streamInfo: com.kuqforza.domain.model.StreamInfo): PlaybackProbeFailure? {
    ...
    private suspend fun shouldProbePlaybackUrl(url: String): Boolean {
        ...
        if (provider.type != com.kuqforza.domain.model.ProviderType.STALKER_PORTAL && cacheKey in probePassedPlaybackKeys) {
            return false
        }
        return (
    ...
    private suspend fun applyRemoteProgramFallback(
        ...
        val programs = (result as? com.kuqforza.domain.model.Result.Success)?.data
            ?.sortedBy { it.startTime }
            .orEmpty()
        ...
        playlistJob = viewModelScope.launch {
            val flows = currentCombinedProfileId?.let { profileId ->
                ...
                if (categoryId == VirtualCategoryIds.RECENT) {
                    ...
                    favoriteRepository.getFavorites(currentProviderId, com.kuqforza.domain.model.ContentType.LIVE)
    ...
    private fun observeCombinedLivePlaylist(profileId: Long, categoryId: Long): Flow<List<com.kuqforza.domain.model.Channel>> = when {
    ...
    private fun applyCombinedSourceProviderFilter(channels: List<com.kuqforza.domain.model.Channel>): List<com.kuqforza.domain.model.Channel> {
    ...
    private fun loadLiveChannelsByOrderedIds(ids: List<Long>, providerId: Long? = null): Flow<List<com.kuqforza.domain.model.Channel>> =
        if (ids.isEmpty()) {
            flowOf(emptyList())
        } else {
    ...
    private fun updateChannelDiagnostics(channel: com.kuqforza.domain.model.Channel) {
    ...
    private fun tryAlternateStreamInternal(channel: com.kuqforza.domain.model.Channel): Boolean {
    ...
    private fun nextLiveVariant(channel: com.kuqforza.domain.model.Channel): com.kuqforza.domain.model.LiveChannelVariant? {
    ...
    private suspend fun startCatchUpPlayback(
        ...
        val catchupStream = com.kuqforza.domain.model.StreamInfo(
            url = primaryUrl,
            title = currentTitle,
            streamType = com.kuqforza.domain.model.StreamType.UNKNOWN
        )
    ...
    private fun showRetryNotice(status: com.kuqforza.player.PlayerRetryStatus) {
    ...
    internal suspend fun resolvePlaybackStreamInfo(
    ...
    ): com.kuqforza.domain.model.StreamInfo? {
        ...
        try {
            xtreamStreamUrlResolver.resolveWithMetadata(
                ...
                return com.kuqforza.domain.model.StreamInfo(
                    url = resolved.url,
                    title = currentTitle,
                    headers = resolved.headers,
                    userAgent = resolved.userAgent,
                    streamType = com.kuqforza.domain.model.StreamType.fromContainerExtension(ext),
        ...
        return logicalUrl.takeIf { it.isNotBlank() }?.let {
            com.kuqforza.domain.model.StreamInfo(
                url = it,
                title = currentTitle,
                streamType = com.kuqforza.domain.model.StreamType.fromContainerExtension(fallbackContainerExtension),
