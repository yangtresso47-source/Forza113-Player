#!/usr/bin/env python3
"""
Add web admin panel to Kuqforza IPTV Premium:
1. NanoHTTPD web server in Kotlin
2. Beautiful HTML admin page
3. API endpoints for portal CRUD
"""
...
if 'nanohttpd' not in build:
    ...
    with open(build_path, 'w') as f:
        f.write(build)
    print('1. Added NanoHTTPD dependency')

# ═══ 2. Create WebAdminServer.kt ═══
server_dir = 'app/src/main/java/com/kuqforza/iptv/web'
os.makedirs(server_dir, exist_ok=True)

with open(os.path.join(server_dir, 'WebAdminServer.kt'), 'w') as f:
    f.write('''package com.kuqforza.iptv.web
...
''')
print('2. WebAdminServer.kt created')

# ═══ 3. Create WebAdminService.kt (foreground service) ═══
with open(os.path.join(server_dir, 'WebAdminService.kt'), 'w') as f:
    f.write('''package com.kuqforza.iptv.web
...
''')
print('3. WebAdminService.kt created')

# ═══ 4. Register service in AndroidManifest ═══
manifest_path = 'app/src/main/AndroidManifest.xml'
with open(manifest_path, 'r') as f:
    manifest = f.read()

if 'WebAdminService' not in manifest:
    old_closing = '</application>'
    new_closing = '''
        <service
            android:name=".web.WebAdminService"
            android:exported="false"
            android:foregroundServiceType="dataSync" />

    </application>'''
    manifest = manifest.replace(old_closing, new_closing, 1)
    with open(manifest_path, 'w') as f:
        f.write(manifest)
    print('4. Service registered in manifest')

# ═══ 5. Auto-start server in MainActivity ═══
main_path = 'app/src/main/java/com/kuqforza/iptv/MainActivity.kt'
if os.path.exists(main_path):
    with open(main_path, 'r') as f:
        main = f.read()
    
    if 'WebAdminService' not in main:
        # Add import
...
import com.kuqforza.iptv.web.WebAdminService'''
        ...
        with open(main_path, 'w') as f:
            f.write(main)
        print('5. Auto-start in MainActivity')

# ═══ 6. Add FOREGROUND_SERVICE permission ═══
if 'FOREGROUND_SERVICE' not in manifest:
    with open(manifest_path, 'r') as f:
        manifest = f.read()
    old_perm = '<uses-feature'
    new_perm = '<uses-permission android:name="android.permission.FOREGROUND_SERVICE" />\n    <uses-permission android:name="android.permission.FOREGROUND_SERVICE_DATA_SYNC" />\n    <uses-feature'
    manifest = manifest.replace(old_perm, new_perm, 1)
    with open(manifest_path, 'w') as f:
        f.write(manifest)
    print('6. Permissions added')

print('\nDone! Access admin panel at http://[TV-IP]:8089')
print('Push with: git add -A && git commit -m "web admin panel" && git push')
